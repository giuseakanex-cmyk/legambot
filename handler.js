...
export async function handler(chatUpdate) {
    ...
    // =======================================================
    // 👑 IL SISTEMA DI GERARCHIA INFALLIBILE (Legam OS)
    // =======================================================
    ...
    if (m.isGroup) {
        ...
        if (groupMetadata) {
            ...
            isBotAdmin = participants.some(u => this.decodeJid(u.id) === this.decodeJid(this.user.jid) && (u.admin === 'admin' || u.admin === 'superadmin'))
        }
    }
    ...
    for (let name in global.plugins) {
        ...
        if (plugin.mods && !isMods) { fail('mods', m, this); continue } // <-- AGGIUNTO SUPPORTO MODERATORI
