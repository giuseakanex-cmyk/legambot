    ...
    try {
        ...
        for (let i = 0; i < users.length; i++) {
            ...
            if (isOwner && isMute) {
                ...
                mutedUsers.set(normalizedPunish, { timestamp: Date.now() + (2 * 60000), reason: 'Hai osato provare a mutare il Creatore 👀', lastNotification: 0, displayNumber: conn.decodeJid(m.sender).split('@')[0] })
            ...
            if (isMute) {
                const muteData = { timestamp: time ? Date.now() + time : 0, reason, lastNotification: 0, displayNumber };
