...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    // ==========================================
    // COMANDO: .crazytime (Apre la lobby)
    // ==========================================
    if (command === 'crazytime' || command === 'ct') {
        if (global.crazyTime[chatId]) return m.reply(`『 🛑 』 \`C'è già una ruota in movimento! Aspetta il risultato.\``)
...
『 🎡 』 𝐋 𝐎 𝐁 𝐁 𝐘  𝐕 𝐈 𝐏
Fate le vostre puntate! La ruota sta per essere azionata dal sistema Legam.
...
💡 Scrivi: *${usedPrefix}betct [Segmento] [Euro]*
𝐄𝐬𝐞𝐦𝐩𝐢𝐨: *${usedPrefix}betct crazy 5000*

⏱️ 𝐂𝐡𝐢𝐮𝐬𝐮𝐫𝐚: 45 Secondi
👑 𝐎𝐖𝐍𝐄𝐑 ➤ 𝐆𝐈𝐔𝐒𝚵`.trim()
        ...
        await conn.sendMessage(chatId, { 
            ...
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    ...
                    newsletterName: "🎡 Legam OS Casino",
                    serverMessageId: 100
                }
            }
        })

        global.crazyTime[chatId].timer = setTimeout(() => startSpin(conn, chatId), 45000)
        return
    }
...
async function startSpin(conn, chatId) {
    ...
    // Se la Top Slot ha colpito il segmento uscito, moltiplichiamo!
    let bonusTS = (tsHit && tsSegment === result.name) ? tsMultiplier : 1

    let resMsg = `🎡 La ruota si ferma su... \n\n✨ [ **${result.name}** ] ✨`
    ...
    if (isBonus) {
        ...
        await finalizeGame(conn, chatId, result.name, finalMultiplier * bonusTS, game)
    }
}
...
async function handleBonus(conn, chatId, type, game, ts) {
    ...
    switch(type) {
        ...
        case 'PACHINKO':
            bonusName = "🏮 𝐏𝐀𝐂𝐇𝐈𝐍𝐊𝐎"
            await conn.sendMessage(chatId, { text: `🚀 Inizio **${bonusName}**!\nLa pallina sta cadendo tra i chiodini...` })
            await new Promise(r => setTimeout(r, 6000))
        ...
        case 'CASH':
            bonusName = "🎯 𝐂𝐀𝐒𝐇 𝐇𝐔𝐍𝐓"
            await conn.sendMessage(chatId, { text: `🚀 Inizio **${bonusName}**!\nPrendi la mira sul tabellone...` })
            await new Promise(r => setTimeout(r, 5000))
            winMul = Math.floor(Math.random() * 90) + 10
            await conn.sendMessage(chatId, { text: `💥 BOOM! Hai colpito un moltiplicatore da **${winMul}x**!` })
            break

        case 'CRAZY':
            bonusName = "🌈 𝐂𝐑𝐀𝐙𝐘 𝐓𝐈𝐌𝐄"
            await conn.sendMessage(chatId, { text: `🔥 BENVENUTI NEL MONDO **CRAZY TIME**! 🔥\nLa ruota gigante sta girando...` })
            await new Promise(r => setTimeout(r, 8000))
            winMul = [50, 100, 150, 200, 500, 1000][Math.floor(Math.random() * 6)]
            await conn.sendMessage(chatId, { text: `👑 INCREDIBILE! Il segmento segna: **${winMul}x**!` })
            break
    }

    await finalizeGame(conn, chatId, type, winMul * ts, game)
}

// ==========================================
// CHIUSURA E PAGAMENTI
// ==========================================
async function finalizeGame(conn, chatId, resultName, totalMul, game) {
    let winners = ""
    ...
    for (let b of game.bets) {
        if (b.pick === resultName) {
            ...
            global.db.data.users[b.sender].euro += (winAmount + b.amount) // Ritorna puntata + vincita
...
✦ ⁺ . ⁺ ✦ ⁺ . ⁺ ✦ ⁺ . ⁺ ✦`.trim()
    ...
    await conn.sendMessage(chatId, { 
        ...
        contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363233544482011@newsletter',
                newsletterName: "🎰 Legam OS Results",
                serverMessageId: 100
            }
        }
    })
